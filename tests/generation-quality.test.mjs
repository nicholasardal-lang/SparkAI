...
const waterRequest='Make a flat blue floor with water and add a splash effect and ripples when I jump and step';
assert.throws(()=>renderBuild(JSON.stringify(build),waterRequest),/no event connection/);
assert.ok(renderBuild(JSON.stringify(build),'Make a flat blue floor'));
for(const location of ['Workspace','StarterPlayer']) {
  assert.throws(()=>renderBuild(JSON.stringify({...build,scripts:[{...script,type:'LocalScript',location}]})),/cannot run directly/);
}
for(const location of ['ServerStorage','ReplicatedStorage','StarterGui']) {
  assert.throws(()=>renderBuild(JSON.stringify({...build,scripts:[{...script,location}]})),/cannot start/);
}
assert.ok(renderBuild(JSON.stringify({...build,scripts:[{...script,type:'ModuleScript',location:'ReplicatedStorage'}]})));
