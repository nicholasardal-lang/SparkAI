export type SparkFile = {name:string; content:string; kind:"script"|"model"; location:string; source:string; scriptType?:string};
...
export function modelFile(source:string):SparkFile {
  ...
  return {name:filename(value.name)+".rbxmx",kind:"model",location:"Workspace",source,content:document(`<Item class="Model" referent="RBX0"><Properties><string name="Name">${xml(value.name)}</string></Properties>${parts}</Item>`)};
}
export function scriptFile(source:string,index=1):SparkFile {
  ...
  return {name:filename(supplied||`SparkScript${index}`)+suffix,kind:"script",scriptType,source,content:source,location:source.match(/^--\s*Spark location:\s*(.+)$/m)?.[1]?.trim() || "See the response for placement instructions"};
}
