...
function colorVector(value:unknown):number[] {
  const color=vector(value,0,255);
  // Models sometimes return Color3-style normalized RGB despite the artifact
  // contract using byte RGB. Treat an entirely 0–1 triplet as normalized so a
  // requested blue does not become almost black when exported to Roblox.
  return color.every(channel=>channel<=1)?color.map(channel=>channel*255):color;
}
...
export function modelFile(source:string):SparkFile {
  ...
  const parts=value.parts.map((p:any,i:number)=>{
    ...
    const size=vector(p.size,.05,2048),position=vector(p.position,-10000,10000),color=colorVector(p.color);
