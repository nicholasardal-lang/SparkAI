...
function AssetForm({ onSaved, projectId }: { onSaved: (asset: LibraryAsset) => void; projectId: string }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [kind, setKind] = useState<"mesh" | "model">("mesh");
  ...
  return <form className="asset-form" onSubmit={save}>
    <div className="asset-form-pair">
      <label>Asset name<input name="name" required maxLength={80} placeholder="Weathered oak barrel" autoFocus /></label>
    ...
    <label>Roblox {kind === "mesh" ? "mesh" : "model"} asset ID<input name="robloxId" required inputMode="numeric" pattern="[1-9][0-9]{0,15}" maxLength={16} placeholder="Enter an existing asset ID" aria-describedby="asset-id-help" /></label>
    ...
    <label>Source link <span>(optional)</span><input name="sourceUrl" type="url" pattern="https://.*" maxLength={1000} placeholder="https://create.roblox.com/…" /></label>
...
export default function AssetStudio({ projectId, initialPrompt = "" }: { projectId: string; initialPrompt?: string }) {
  ...
  return <section className="asset-studio" aria-label="Asset Studio">
    <div className="asset-studio-heading"><div><span className="asset-kicker"><Layers3 size={14} aria-hidden="true" /> PROJECT ASSET STUDIO</span><h2>Build with real assets.</h2><p>Give Spark a reusable library of meshes and models. Get a structured scene plan and a reviewed path into Roblox Studio.</p></div><button className="asset-secondary" disabled={!!working || loading} onClick={() => void action("refresh", load)}><RefreshCw size={15} className={working === "refresh" ? "asset-spin" : ""} /> Refresh</button></div>
    <div className="asset-pipeline" aria-label="Workflow"><span><b>1</b> Add assets</span><ArrowRight size={14} aria-hidden="true" /><span><b>2</b> Plan the scene</span><ArrowRight size={14} aria-hidden="true" /><span><b>3</b> Review & import</span></div>
    ...
    <Dialog open={addOpen} onOpenChange={setAddOpen}><DialogContent className="asset-dialog"><DialogTitle>Add a reusable asset</DialogTitle><DialogDescription>Register an existing Roblox asset for this project. This saves a reference; it does not upload, purchase, or generate an asset.</DialogDescription><AssetForm projectId={projectId} onSaved={asset => { setAssets(current => [asset, ...current]); setAddOpen(false); setNotice(`${asset.name} added. New plans can use it; existing plans are unchanged.`); }} /></DialogContent></Dialog>
  </section>;
}
